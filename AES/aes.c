...
#define Nb 4
#define SIXTEEN 16 //Block length in bytes AES is 128b block only
...
// Esta função adiciona a chave redonda ao estado.
// A tecla redonda é adicionada ao estado por uma função XOR.
...
// A função SubBytes Substitui os valores no
// matriz de estado com valores em uma caixa S.
...
// A função ShiftRows () desloca as linhas no estado para a esquerda.
// Cada linha é deslocada com deslocamento diferente.
// Offset = Número da linha. Portanto, a primeira linha não é alterada.
...
// A função MixColumns mistura as colunas da matriz de estado
...
// A cifra é a principal função que criptografa o PlainText.
// Cipher is the main function that encrypts the PlainText.
static void Cipher(void)
{
    uint8_t round = 0;

// Adicione a primeira tecla redonda ao estado antes de iniciar as rodadas.
...
void AES_ECB_encrypt(const uint8_t* input, const uint8_t* key, uint8_t* output, const uint32_t length){
    ...
    // A próxima chamada de função criptografa o PlainText com a chave usando o algoritmo AES.
